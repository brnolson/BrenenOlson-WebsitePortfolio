...
const PinScene = ({ progress }) => {
  ...
  React.useEffect(() => {
    ...
    const cytoMat = track(new THREE.MeshBasicMaterial({
      color: CELL_COLORS.cyto, transparent: true, opacity: 0.14, depthWrite: false, side: THREE.BackSide,
    }));
    ...
    const memWireMat = track(new THREE.MeshBasicMaterial({ color: CELL_COLORS.membraneWire, wireframe: true, transparent: true, opacity: 0.18 }));
    ...
    const bilayerMat = track(new THREE.PointsMaterial({
      color: CELL_COLORS.bilayerDots, size: 0.032, transparent: true, opacity: 0.75,
    ...
    const nucWireMat = track(new THREE.MeshBasicMaterial({ color: CELL_COLORS.nucleusWire, wireframe: true, transparent: true, opacity: 0.35 }));
    ...
    const poreMat = track(new THREE.PointsMaterial({
      color: CELL_COLORS.nuclearPore, size: 0.07, transparent: true, opacity: 0.95,
      blending: THREE.AdditiveBlending, depthWrite: false,
    }));
    ...
    cellGroup.add(er);
    const erWireMat = track(new THREE.MeshBasicMaterial({ color: CELL_COLORS.erWire, wireframe: true, transparent: true, opacity: 0.28 }));
    ...
    for (let gi = 0; gi < 3; gi++) {
      ...
      golgiGroup.add(disk);
      const wm = track(new THREE.MeshBasicMaterial({ color: CELL_COLORS.golgiWire, wireframe: true, transparent: true, opacity: 0.28 }));
    ...
    const vesicleMat = track(new THREE.PointsMaterial({
      color: CELL_COLORS.vesicles, size: 0.08, transparent: true, opacity: 0.85,
      blending: THREE.AdditiveBlending, depthWrite: false,
    }));
    ...
    for (let mi = 0; mi < mtCount; mi++) {
      ...
      const m = track(new THREE.LineBasicMaterial({ color: CELL_COLORS.cytoskeleton, transparent: true, opacity: 0.22 }));
    ...
    let placed = 0;
    while (placed < riboN) {
      const x = (Math.random()-0.5)*4.4, y = (Math.random()-0.5)*4.4, z = (Math.random()-0.5)*4.4;
      const r = Math.sqrt(x*x+y*y+z*z);
      if (r > 1.2 && r < 2.35) { riboPos[placed*3]=x; riboPos[placed*3+1]=y; riboPos[placed*3+2]=z; placed++; }
    }
    riboGeo.setAttribute("position", new THREE.BufferAttribute(riboPos, 3));
    const riboMat = track(new THREE.PointsMaterial({
      color: CELL_COLORS.ribosomes, size: 0.05, transparent: true, opacity: 0.9,
      blending: THREE.AdditiveBlending, depthWrite: false,
    }));
    ...
    const cellAnchors = [
      { id: "M",  title: "Plasma Membrane", subtitle: "Phospholipid bilayer",  local: [ 2.42,  0.55,  0.0 ], pIn: 0.10, pOut: 0.45 },
      { id: "N",  title: "Nucleus",         subtitle: "DNA · chromosomes",     local: [ 0.0,   0.95,  0.0 ], pIn: 0.36, pOut: 0.68 },
      { id: "Mi", title: "Mitochondrion",   subtitle: "ATP synthesis",         local: [ 1.40,  0.78,  0.95], pIn: 0.58, pOut: 0.92 },
      { id: "G",  title: "Golgi Apparatus", subtitle: "Vesicle packaging",     local: [-1.50,  0.95,  0.65], pIn: 0.78, pOut: 1.10 },
      { id: "R",  title: "Ribosomes",       subtitle: "Protein synthesis",     local: [ 1.70, -1.50,  0.50], pIn: 0.95, pOut: 1.28 },
    ];
    ...
    const DNA_HEIGHT = 8, TURNS = 3, DNA_RADIUS = 0.6, SEG = 130;
    const RUNG_COUNT = 28;
    ...
    const dnaAnchors = [
      { id: "BB", title: "Sugar-Phosphate", subtitle: "Backbone strand",    pIn: 1.75, pOut: 2.45, local: [ 1.0,  0.5, 0] },
      { id: "BP", title: "Base Pair",       subtitle: "A·T or G·C",         pIn: 2.20, pOut: 2.90, local: [-0.8, -0.5, 0] },
      { id: "5",  title: "5′ End",          subtitle: "Phosphate terminus",  pIn: 2.55, pOut: 3.20, local: [ 0.5,  2.0, 0] },
      { id: "MG", title: "Major Groove",    subtitle: "Wide face · 22 Å",   pIn: 3.10, pOut: 3.65, local: [ 1.2,  0.8, 0] },
      { id: "mg", title: "Minor Groove",    subtitle: "Narrow face · 12 Å", pIn: 3.40, pOut: 3.85, local: [-1.0, -0.3, 0] },
      { id: "3",  title: "3′ End",          subtitle: "Hydroxyl terminus",   pIn: 3.62, pOut: 4.00, local: [-0.5, -2.0, 0] },
    ];
    ...
    [0, 1].forEach(s => {
      ...
      const bpMat = track(new THREE.PointsMaterial({
        color: DNA_COLORS.phosphate, size: 0.14, transparent: true, opacity: 0.95,
        blending: THREE.AdditiveBlending, depthWrite: false,
      }));
    ...
    for (let i = 0; i < RUNG_COUNT; i++) {
      ...
      const wm = track(new THREE.LineBasicMaterial({ color: DNA_COLORS.rungWire, transparent: true, opacity: 0.45 }));
    ...
    const makeStarLayer = (count, zNear, zFar, size, baseColor) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        col[i*3]=r*tint; col[i*3+1]=g*tint; col[i*3+2]=b*tint;
      }
      ...
      if (cellFade > 0.02) {
        for (const a of cellAnchors) {
          ...
          if (fade > 0.02) {
            ...
            if (pos) labels.push({ id: a.id, title: a.title, subtitle: a.subtitle, x: pos.x, y: pos.y, fade });
          }
        }
      }
