...
const PinScene = ({ progress }) => {
  ...
  React.useEffect(() => {
    ...
    const glbMats = [];
    let glbMixer = null, glbMixer2 = null;
    let surfHalfH = 11, surfRad = 5;
    ...
    const applyHolo = (root) => {
      ...
      root.traverse(obj => {
        ...
        const mat = new THREE.MeshBasicMaterial({
          color: 0x00ccff, transparent: false,
    ...
    if (typeof THREE.GLTFLoader !== "undefined") {
      new THREE.GLTFLoader().load(
        "assets/models/dna_hologram.glb",
        (gltf) => {
          ...
          if (sz0.x === mx0) model.rotation.z =  Math.PI / 2;
          else if (sz0.z === mx0) model.rotation.x = -Math.PI / 2;
          model.updateMatrixWorld(true);
          ...
          const TARGET_H = 26;
          const s = Math.max(sz1.x, sz1.y, sz1.z) > 0.0001
            ? TARGET_H / Math.max(sz1.x, sz1.y, sz1.z) : 1;
        ...
        (xhr) => { if (xhr.total) console.log("[pin-scene] GLB", Math.round(xhr.loaded / xhr.total * 100) + "%"); },
    ...
    const dnaAnchors = [
      { id:"BB", title:"Sugar-Phosphate Backbone", subtitle:"Phosphodiester · 2 nm diameter", pIn:2.8, pOut:3.2, x:72, y:18 },
      { id:"MG", title:"Major Groove",             subtitle:"22 Å wide · protein landing pad", pIn:3.0, pOut:3.5, x:68, y:42 },
      { id:"BP", title:"Base Pair",                subtitle:"A·T × 2 bonds · G·C × 3 bonds",  pIn:3.2, pOut:3.7, x:14, y:56 },
      { id:"mG", title:"Minor Groove",             subtitle:"12 Å wide · 7.5 Å deep",          pIn:3.5, pOut:4.0, x:18, y:78 },
    ];
    ...
    const makeStarLayer = (count, zNear, zFar, sz, baseColor) => {
      ...
      const r = ((baseColor >> 16) & 0xff) / 255, g = ((baseColor >> 8) & 0xff) / 255, b = (baseColor & 0xff) / 255;
      ...
      const mat = track(new THREE.PointsMaterial({ vertexColors:true, size:sz, transparent:true, opacity:0.9, blending:THREE.AdditiveBlending, depthWrite:false }));
    ...
    const loop = (t) => {
      ...
      //        so it follows the model as it rotates. Zero discontinuities.
      {
        ...
        const ridePos  = new THREE.Vector3(
          surfHalfH * (-0.85 + rideT * 0.85),             // left end of strand → center
          Math.sin(helixAng) * droneDist,
          Math.cos(helixAng) * droneDist,
        );
        ...
        const glidePos = new THREE.Vector3(
          -localY * Math.sin(rotZ) + dnaX * 0.18,
           localY * Math.cos(rotZ),
           glideZ,
        );
      ...
      if (p > ROT_END) {
        for (const a of dnaAnchors) {
          ...
          if (fade > 0.02) labels.push({ id:a.id, title:a.title, subtitle:a.subtitle, x:a.x, y:a.y, fade });
        }
      }
    ...
    if (REDUCED) { renderer.render(scene, camera); } else { raf = requestAnimationFrame(loop); }

    return () => {
      cancelAnimationFrame(raf); ro.disconnect(); vio.disconnect(); renderer.dispose();
...
const PinLabels = () => {
  ...
  return (
    <div className="pin-labels">
      {labels.map(L => (
        <div key={L.id} className="pin-label" style={{ left:`${L.x}%`, top:`${L.y}%`, opacity:L.fade }}>
          <div className="pin-label-dot"/>
          <div className="pin-label-line"/>
          <div className="pin-label-box">
            <div className="pin-label-id">{L.id}</div>
            <div><div className="pin-label-title">{L.title}</div><div className="pin-label-sub">{L.subtitle}</div></div>
          </div>
        </div>
      ))}
    </div>
