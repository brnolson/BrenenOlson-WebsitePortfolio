...
const PinScene = ({
  ...
  React.useEffect(() => {
    ...
    const makeHalfRung = (type /* "A" | "T" */) => {
      ...
      const sugMat = track(new THREE.MeshBasicMaterial({
        color: DNA_COLORS.sugar,
        transparent: true,
        opacity: 0.9
      }));
      ...
      const sugWireMat = track(new THREE.LineBasicMaterial({
        color: DNA_COLORS.rungWire,
        transparent: true,
        opacity: 0.7
      }));
      ...
      const pWireMat = track(new THREE.LineBasicMaterial({
        color: DNA_COLORS.phosphateWire,
        transparent: true,
        opacity: 0.7
      }));
      ...
      const oMat = track(new THREE.MeshBasicMaterial({
        color: CPK_OXYGEN,
        transparent: true,
        opacity: 0.95
      }));
      ...
      const bondMat = track(new THREE.MeshBasicMaterial({
        color: PO_BOND,
        transparent: true,
        opacity: 0.7
      }));
      ...
      const oWireMat = track(new THREE.LineBasicMaterial({
        color: 0xffdada,
        transparent: true,
        opacity: 0.6
      }));
      ...
      const gbondMat = track(new THREE.MeshBasicMaterial({
        color: GLYC_BOND,
        transparent: true,
        opacity: 0.75
      }));
    ...
    const hbondMat = track(new THREE.MeshBasicMaterial({
      color: DNA_COLORS.hbond,
      transparent: true,
      opacity: 0.6
    }));
    ...
    const loop = t => {
      ...
      const nucleotideCamPos = nucMid.clone().add(nucPerp.clone().multiplyScalar(0.7)).add(new THREE.Vector3(0, 0.14, 0));
      ...
      const WP = {
        ...
        rungSide: nucleotideCamPos,
        // perpendicular side view of focus rung
      ...
      if (p < 1) {
        ...
        const approach = new THREE.Vector3(focusWorldPos.x * 0.35, focusWorldPos.y * 0.3, 3.4);
        if (p < 2.4) {
      ...
      if (pairFade > 0.02) {
        ...
        if (pA) labels.push({
          id: "A",
          title: "Adenine",
          subtitle: "Nitrogenous base · purine",
          x: pA.x,
          y: pA.y,
          fade: pairFade
        });
        if (pT) labels.push({
          id: "T",
          title: "Thymine",
          subtitle: "Nitrogenous base · pyrimidine",
          x: pT.x,
          y: pT.y,
          fade: pairFade
        });
